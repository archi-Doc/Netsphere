...
public class RemoteBenchSubcommand : ISimpleCommand<RemoteBenchOptions>
{
    ...
    public async Task Execute(RemoteBenchOptions options, string[] args, CancellationToken cancellationToken)
    {
        ...
        try
        {
            using var cts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, connection.CancellationToken);
            await Task.Delay(TimeSpan.FromMinutes(5)).WaitAsync(cts.Token);
        }
        catch
        {
        }
